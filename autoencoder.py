...
def search_walk(dirName):
    ...
    for i in folders:
        for dpath, dname, filename in os.walk(os.path.join(dirName,i)):
...
def stack_np(list, idx):
    ...
    np_st = np.stack(
        [read1, read2, read3, read4, read5, read6, read7, read8, read9, read10, read11, read12, read13, read14], axis=0)
...
path = 'abnormal-frames-maskr-112'
...
for epoch in range(Epocs):
    for data in trn_data:
        ...
        if epoch % 10 == 0:
            torch.save(ae.state_dict(), 'output/flow-ae-feature-{}-{}.pth'.format('210427', epoch))

    print('epoch [{}/{}], loss : {:.6f}'.format(epoch + 1, Epocs, loss_a.data))

    losses_a.append(loss_a.data)
torch.save(ae.state_dict(), 'output/latest-flow-ae-feature-{}-{}.pth'.format('210427', epoch))
